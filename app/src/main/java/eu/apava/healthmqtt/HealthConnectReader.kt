...
class HealthConnectReader(
    ...
    suspend fun readSnapshot(): HealthSnapshot {
        ...
        debugRecordScan<StepsRecord>("scan_steps_today_overlaps", TimeRangeFilter.overlaps(startOfToday, now), debug)
        debugRecordScan<StepsRecord>("scan_steps_7d", TimeRangeFilter.between(now.minus(Duration.ofDays(7)), now), debug)
        debugRecordScan<DistanceRecord>("scan_distance_7d", TimeRangeFilter.overlaps(now.minus(Duration.ofDays(7)), now), debug)
        debugRecordScan<ActiveCaloriesBurnedRecord>("scan_active_calories_7d", TimeRangeFilter.overlaps(now.minus(Duration.ofDays(7)), now), debug)
        debugRecordScan<TotalCaloriesBurnedRecord>("scan_total_calories_7d", TimeRangeFilter.overlaps(now.minus(Duration.ofDays(7)), now), debug)
        debugRecordScan<HeartRateRecord>("scan_heart_rate_30d", TimeRangeFilter.between(last30Days, now), debug)
        debugRecordScan<SleepSessionRecord>("scan_sleep_30d", TimeRangeFilter.overlaps(last30Days, now), debug)
        ...
        return try {
            val response = client.readRecords(
                ReadRecordsRequest(
                    ...
                    timeRangeFilter = TimeRangeFilter.overlaps(start, end)
                )
            )
            ...
            debug["steps_source"] = "records_sum_overlaps"
    ...
    private suspend fun readDistanceMeters(start: Instant, end: Instant, debug: MutableMap<String, String>): Double? {
        ...
        return try {
            ...
            debug["distance_source"] = "records_sum_overlaps"
    ...
    private suspend fun readActiveCaloriesKcal(start: Instant, end: Instant, debug: MutableMap<String, String>): Double? {
        ...
        return try {
            ...
            debug["active_calories_source"] = "records_sum_overlaps"
    ...
    private suspend fun readFloors(start: Instant, end: Instant, debug: MutableMap<String, String>): Double? {
        ...
        return try {
            ...
            debug["floors_source"] = "records_sum_overlaps"
